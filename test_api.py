...
class Zlibrary:
    ...
    def getBookFile(self, bookid: [int, str], hashid: str) -> [(str, bytes), None]:
...
def main():
    ...
    try:
        ...
        # 2. 搜索书籍（示例：搜索“Python编程”，可自定义参数）
        search_result = zlib.search(
            message="Python编程",  # 搜索关键词
            extensions=["epub", "pdf"],  # 指定格式
            limit=5  # 返回结果数量
        )
        if not search_result.get("books"):
            print("未找到相关书籍")
            return
        ...
        if book_info:
            ...
            # 从返回的响应中解析下载直链（也可以直接看 getBookFile 方法里的 ddl 变量）
            # 如果你只想拿直链，可修改 getBookFile 方法返回 ddl
            print(f"书籍文件名：{filename}")
            print(f"下载直链（临时有效）：{zlib._Zlibrary__makeGetRequest(f'/eapi/book/{book["id"]}/{book["hash"]}/file')['file']['downloadLink']}")

        # 方式2：直接下载书籍到本地（可改写成上传到坚果云）
        # filename, file_content = zlib.downloadBook(book)
        # with open(filename, "wb") as f:
        #     f.write(file_content)
        # print(f"书籍已下载到本地：{filename}")

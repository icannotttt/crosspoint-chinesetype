...
// gd:新加局部清空指定区域的显存（和clearScreen逻辑一致，仅限定区域）
void EInkDisplay::clearArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t color) const {
    if (!frameBuffer) {
    ...
    if (x >= DISPLAY_WIDTH || y >= DISPLAY_HEIGHT || w == 0 || h == 0) {
        Serial.printf("[%lu]   ERROR: Invalid clearArea bounds (%d,%d,%dx%d)\n", millis(), x, y, w, h);
        return;
    }
    ...
    // 2. 局部memset（核心：仅清空指定区域的frameBuffer，和clearScreen逻辑对齐）
    // 计算：1字节=8像素，先转成字节坐标
    const uint16_t x_byte_start = x / 8;
    const uint16_t x_byte_end = x_end / 8;
    const uint16_t byte_width = x_byte_end - x_byte_start + 1;

    // 逐行memset（只处理指定Y范围，和clearScreen的全屏memset逻辑一致）
    for (uint16_t row = y; row <= y_end; row++) {
        uint32_t buffer_offset = row * DISPLAY_WIDTH_BYTES + x_byte_start;
        memset(&frameBuffer[buffer_offset], color, byte_width);
    }

    Serial.printf("[%lu]   Cleared area (%d,%d,%dx%d) with color 0x%02X\n", millis(), x, y, w, h, color);
}

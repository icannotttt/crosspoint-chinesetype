...
void GfxRenderer::drawBitmap(const Bitmap& bitmap, const int x, const int y, const int maxWidth,
  ...
  if (!outputRow || !rowBytes) {
        Serial.printf("[%lu] [GFX] - outputRow 需要内存：%u bytes\n", millis(), outputRowSize);
    Serial.printf("[%lu] [GFX] - rowBytes 需要内存：%u bytes\n", millis(), bitmap.getRowBytes());
...
// gd:新加，局部清屏
void GfxRenderer::clearArea(int x, int y, int width, int height, const uint8_t color) const {
    // 调用屏幕驱动的「局部清屏」接口（底层逻辑和全屏清屏一致，仅限定区域）
    einkDisplay.clearArea(x, y, width, height, color); 
}
//gd:新加，局部绘制
void GfxRenderer::displayWindow(int x, int y, int width, int height) const {
    // 核心：调用EInkDisplay的displayWindow（_einkDisplay是GfxRenderer中EInkDisplay的成员变量）
    // 注意：如果你的成员变量名不是_einkDisplay，改成实际名称（比如einkDisplay/_display）
    einkDisplay.displayWindow(static_cast<uint16_t>(x), 

...
class BookMetadataCache {
 ...
 private:
  ...
  //分片管理
  const int page =0;
  const int chapterbegin=page*25;
  const int chapterend=(page+1)*25;
  std::string spineBinFile = "/spine"+std::to_string(page)+".bin.tmp";
  const char*  tmpSpineBinFile=spineBinFile.c_str();
  std::string tocBinFile = "/toc"+std::to_string(page)+".bin.tmp";
  const char*  tmpTocBinFile=tocBinFile.c_str();

    ...
    if (StringUtils::checkFileExtension(lastBookTitle, ".epub")) {
      ...
      if (xtc.load()) {
        ...
        if (xtc.generateCoverBmp()) {
          coverBmpPath = xtc.generateCoverBmp();
          hasCoverImage = true;
        }
      }
...
void HomeActivity::render() {
  ...
  if (hasContinueReading) {
    ...
    if (coverRendered) {
      ...
      renderer.drawCenteredText(UI_10_FONT_ID, continueY, "继续阅读", !bookSelected);
    }
  } else {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, y + renderer.getLineHeight(UI_12_FONT_ID), "现在开始阅读吧！");
  }

  // --- Bottom menu tiles ---
  // Build menu items dynamically
  std::vector<const char*> menuItems = {"挑选一本书", "wifi传书", "设置"};
  if (hasOpdsUrl) {

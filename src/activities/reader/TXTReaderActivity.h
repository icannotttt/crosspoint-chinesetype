...
public:
  ...
  // 2. 彻底删除std::make_unique，改用new（核心修复）
  std::unique_ptr<std::string> loadPageFromSectionFile() {
    if (currentPage < 0 || currentPage >= pageCount) {
      return nullptr;
    }
    // 完全替换make_unique（C++11兼容）
    std::string* content = new std::string(txt->getPage(currentPage));
...
class TXTReaderActivity final : public ActivityWithSubactivity {
  ...
  // TXTReaderActivity.h -> private 区域
  uint32_t currentPage = 0; // 当前阅读页码

...
void FileSelectionActivity::render() const {
  ...
  if (files.empty()) {
    renderer.drawText(UI_FONT_ID, 20, 60, "No files found");
    renderer.displayBuffer();
    return;
  }

  const auto pageStartIndex = selectorIndex / PAGE_ITEMS * PAGE_ITEMS;
  renderer.fillRect(0, 70 + (selectorIndex % PAGE_ITEMS) * 30 + 2, pageWidth - 1, 30);
  for (int i = pageStartIndex; i < files.size() && i < pageStartIndex + PAGE_ITEMS; i++) {
    ...
    renderer.drawText(UI_FONT_ID, 20, 70 + (i % PAGE_ITEMS) * 30, item.c_str(), i != selectorIndex);
  }
